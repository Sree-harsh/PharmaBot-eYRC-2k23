...
def detect_Qr_details(image):
    ...
    for barcode in barcodes:
        ...
        list=[int(x+w/2),int(y+h/2)]
        Qr_codes_details[text]=list
    ##################################################
...
def detect_ArUco_details(image):
    ...
    aruco_corners, aruco_IDs, reject= aruco.detectMarkers(image, aruco_dict, parameters=param_aruco)
    ...
    if aruco_corners:
        ...
        for ids, corners in zip(aruco_IDs, aruco_corners):
            ...
            mid_point = ( int(round((top_right[0]+bottom_left[0])/2)), int(round((top_right[1]+bottom_left[1])/2)) )
            mid_point_topLine = (int(round((top_right[0]+top_left[0])/2)), int(round((top_right[1]+top_left[1])/2)) )

            
            y2 = mid_point_topLine[1]
            y1 = mid_point[1]
            x2 = mid_point_topLine[0]
            x1 = mid_point[0]

            angle_R = math.atan2((y2-y1), (x2-x1))
            angle_degree =round(math.degrees(angle_R))
        

            if x2 > x1 :

                angle = -1*(90 + angle_degree)
                
                
            elif x2 < x1:
                if angle_degree > 0 :
                    angle = 270 - angle_degree
                else:
            ...
            ArUco_corners[ids[0]]=(top_left,top_right,bottom_right,bottom_left)
            ArUco_details_dict[int(ids[0])]=[[x1,y1],angle]
    ##################################################
